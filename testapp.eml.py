...
def extract_eml_content(script_path):
    ...
    try:
        ...
        # Znajdź początek sekcji EML (po komentarzach Python)
        eml_start = content.find('MIME-Version: 1.0')
        if eml_start == -1:
            raise ValueError("Nie znaleziono sekcji EML w pliku")

        eml_content = content[eml_start:]

        # Zapisz EML do pliku tymczasowego
        eml_file = os.path.join(temp_dir, 'content.eml')
        with open(eml_file, 'w', encoding='utf-8') as f:
            f.write(eml_content)

        # Parsuj EML używając biblioteki email
        with open(eml_file, 'rb') as f:
            msg = email.message_from_binary_file(f)

        extracted_files = []

        # Przetwórz każdą część MIME
        for part in msg.walk():
            ...
            if not filename:
                # Spróbuj wyciągnąć z Content-Disposition
                ...
                # Jeśli nadal brak nazwy, określ na podstawie typu
                if not filename:
                    ...
                    if content_type == 'text/html':
                        ...
                        filename = content_id.replace('_css', '.css') if content_id else 'style.css'
                    elif content_type == 'application/javascript':
                        filename = content_id.replace('_js', '.js') if content_id else 'script.js'
                    elif content_type == 'text/plain' and 'dockerfile' in part.get_payload().lower():
                        filename = 'Dockerfile'
            ...
            try:
                ...
                if encoding == 'base64':
                    ...
                    content = quopri.decodestring(payload.encode()).decode('utf-8')
                    mode = 'w'
                else:
                ...
                print(f"❌ Błąd przy wyodrębnianiu {filename}: {e}")
